...
parameters {
  matrix[n_pcs, n_sex] eps[n_code, n_year];      // one pc x sex matrix per county-year
  matrix[n_ages, n_sex] u[n_code, n_year];       // one age x sex error matrix per county-year
  
  matrix[n_pcs, n_sex] mu_beta[n_state, n_year]; //matrix[n_pcs, n_sex] mu_beta[n_state, n_year]; // one pc vector of mu_beta intercepts for year 1
  ...
  cholesky_factor_corr[n_sex] beta_chol[n_pcs, n_year];
  cholesky_factor_corr[n_sex] chol[n_ages, n_year];
...
transformed parameters {
  matrix[n_pcs, n_sex] beta[n_code, n_year];    // one pc x sex coefficient matrix per county-year
  matrix[n_ages, n_sex] logmx[n_code, n_year];  // one age x sex log-mort matrix per county-year  
...
generated quantities {
  corr_matrix[n_sex] cor[n_ages, n_year];
  corr_matrix[n_sex] beta_cor[n_pcs, n_year];
  ...
  // 
  // for (i in 1:n) {
  //   yrepl[i] = poisson_rng(exp(logmx[code[i], year[i]][ages[i], sex[i]]) * pop[i]);
  // }
}
